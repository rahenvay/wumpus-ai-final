...
adjacent(X1, Y1, X2, Y2) :-
    valid_pos(X1, Y1), valid_pos(X2, Y2),
    ( (X2 is X1 + 1, Y2 is Y1);
      (X2 is X1 - 1, Y2 is Y1);
      (X2 is X1, Y2 is Y1 + 1);
      (X2 is X1, Y2 is Y1 - 1) ).
...
% A square (X,Y) is PROVEN not to be a pit if it is adjacent to a visited square that had NO breeze.
not_a_pit(X, Y) :-
    adjacent(X, Y, AX, AY),
    visited(AX, AY),
...
% A square (X,Y) is PROVEN not to be a Wumpus if it is adjacent to a visited square that had NO stench.
not_a_wumpus(X, Y) :-
    adjacent(X, Y, AX, AY),
    visited(AX, AY),
...
% Safety Rules
% Rule 1: A square we have already safely visited is safe.
is_safe(X, Y) :- visited(X, Y).

% Rule 2: An unvisited square is safe if we can logically prove it lacks both a pit and a Wumpus.
is_safe(X, Y) :-
    valid_pos(X, Y),
    \+ visited(X, Y),
    not_a_pit(X, Y),
